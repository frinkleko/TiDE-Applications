...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        res = self.linear_res(
            x)  # [B,L,in_dim] -> [B,L,out_dim] or [B,in_dim] -> [B,out_dim]
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer_num, hidden_dim, r, r_hat, L, H,
                 featureProjectionHidden):
        super(Encoder, self).__init__()
        self.encoder_layer_num = layer_num
        self.horizon = H
        self.feature_projection = ResidualBlock(r, featureProjectionHidden,
                                                r_hat)
        self.first_encoder_layer = ResidualBlock(L + 1 + (L + H) * r_hat,
                                                 hidden_dim, hidden_dim)
        self.other_encoder_layers = nn.ModuleList([
            ResidualBlock(hidden_dim, hidden_dim, hidden_dim)
    ...
    def forward(self, x, covariates, attributes):
        ...
        # Feature Projection
        covariates = self.feature_projection(
            covariates)  # [B*N,L+H,r] -> [B*N,L+H,r_hat]
        covariates_future = covariates[:, -self.horizon:, :]  # [B*N,H,r_hat]

        # Flatten
        covariates_flat = rearrange(
            covariates,
            'b l r -> b (l r)')  # [B*N,L+H,r_hat] -> [B*N,(L+H)*r_hat]

        # Concat
        e = torch.cat([x, attributes, covariates_flat],
                      dim=1)  # [B*N,L+1+(L+H)*r_hat]

        # Dense Encoder
        e = self.first_encoder_layer(
            e)  # [B*N,L+1+(L+H)*r_hat] -> [B*N,hidden_dim]
        for i in range(self.encoder_layer_num - 1):
            e = self.other_encoder_layers[i](
                e)  # [B*N,hidden_dim] -> [B*N,hidden_dim]
...
class Decoder(nn.Module):
    ...
    def __init__(self, layer_num, hidden_dim, r_hat, H, p,
        ...
        self.temporaldecoder = ResidualBlock(p + r_hat, temporalDecoderHidden,
                                             1)
    ...
    def forward(self, e, covariates_future):
        ...
        for i in range(self.decoder_layer_num - 1):
            e = self.other_decoder_layers[i](
                e)  # [B*N,hidden_dim] -> [B*N,hidden_dim]
        g = self.last_decoder_layer(e)  # [B*N,hidden_dim] -> [B*N,p*H]

        # Unflatten
        matrixD = rearrange(g, 'b (h p) -> b h p',
                            h=self.horizon)  # [B*N,p*H] -> [B*N,H,p]

        # Stack
        out = torch.cat([matrixD, covariates_future],
                        dim=-1)  # [B*N,H,p+r_hat]
...
class TiDE(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = Encoder(encoder_layer_num, hidden_dim, r, r_hat, L, H,
                               featureProjectionHidden)
        self.decoder = Decoder(decoder_layer_num, hidden_dim, r_hat, H, p,
                               temporalDecoderHidden)
    ...
    def forward(self, x, covariates, attributes):
        ...
        covariates = rearrange(
            covariates, 'b l n r -> (b n) l r')  # [B,L+H,N,r] -> [B*N,L+H,r]
        ...
        # Reshape
        prediction = rearrange(prediction, '(b n) h -> b h n',
                               b=batch_size)  # [B*N,H] -> [B,H,N]
